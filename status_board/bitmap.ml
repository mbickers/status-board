...
let create ~width ~height =
  { width; height; pixels = Array.create ~len:(width * height) false }
;;

let size t = t.width, t.height

let write_exn t ~x ~y color =
  Array.set
    t.pixels
    ((y * t.width) + x)
    (match color with
...
let encode_bmp bitmap =
  let row_size = (bitmap.width + 31) / 32 * 4 in
  let pixel_data_size = row_size * bitmap.height in
  ...
  set_little_endian result ~offset:18 ~byte_count:4 bitmap.width;
  set_little_endian result ~offset:22 ~byte_count:4 bitmap.height;
  ...
  for output_y = 0 to bitmap.height - 1 do
    let bitmap_y = bitmap.height - output_y - 1 in
    for x = 0 to bitmap.width - 1 do
      match bitmap.pixels.((bitmap_y * bitmap.width) + x) with
      | true ->
