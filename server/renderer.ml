...
let respond ~cache ~(status_board : Status_board.t) request =
  let input =
    ...
    | Some "device" ->
      ...
      in
      Status_board.Input.Device { battery_voltage }
    | Some "preview" ->
      Uri.get_query_param uri "preset"
      |> Option.filter ~f:(Fn.non String.is_empty)
      |> Status_board.Input.Preview
      |> Or_error.return
    | Some input -> Or_error.errorf "Unknown status board input %S" input
    | None -> Or_error.error_string "Status board image request has no input"
  in
  let%bind rendered =
    match input with
