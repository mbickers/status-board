...
let write buffer x y color =
  if x >= 0 && y >= 0 && x < buffer.Image.width && y < buffer.height
  then Image.write_grey buffer x y color
;;

let rect buffer ~color (x1, y1) (x2, y2) =
  for y = y1 to y2 - 1 do
    for x = x1 to x2 - 1 do
      write buffer x y color
    done
  done
;;

let rounded_path buffer ~radius ~stroke ~stroke_width points =
  let distance (x1, y1) (x2, y2) =
    Float.sqrt (((x2 -. x1) *. (x2 -. x1)) +. ((y2 -. y1) *. (y2 -. y1)))
  in
  let stroke_radius = Float.of_int stroke_width /. 2. in
  let draw_point (center_x, center_y) =
    for
    ...
    do
      ...
      do
        ...
        then write buffer x y stroke
      done
    done
  in
  let draw_line ((x1, y1) as start) ((x2, y2) as finish) =
    let steps =
      Int.max 1 (distance start finish *. 2. |> Float.round_up |> Int.of_float)
    in
    for step = 0 to steps do
      let progress = Float.of_int step /. Float.of_int steps in
      draw_point (x1 +. ((x2 -. x1) *. progress), y1 +. ((y2 -. y1) *. progress))
    done
  in
  let draw_curve start control finish =
    let steps =
      Int.max
        1
  ...
  | first :: points ->
    let rec draw current previous = function
      | [] -> ()
      | [ last ] -> draw_line current last
      | ((vertex_x, vertex_y) as vertex) :: (next :: _ as remaining) ->
        ...
        in
        let curve_start = tangent_point previous previous_length
        and curve_end = tangent_point next next_length in
        draw_line current curve_start;
        draw_curve curve_start vertex curve_end;
...
let draw ~font =
  ...
  text buffer ~font ~origin_x:222 ~baseline_y:100 ~size:80. "hello world";
  let map_top = h / 2 in
  let man_w = 250 in
